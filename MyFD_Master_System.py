...
st.setpageconfig(pagetitle="FlashDeal Star Universal", pageicon="🌟", layout="wide")
...
with st.sidebar:
    ...
    selectedlang = st.selectbox("🌐 Global Language", list(LANGDICT.keys()))
    t = LANGDICT[selectedlang]
    ...
    with st.expander(t['mem'], expanded=True):
        if not st.session_state.history:
            ...
            for item in reversed(st.session_state.history):
                st.markdown(f"<p class='log-text'>{item}</p>", unsafeallowhtml=True)
    
    st.divider()
    acc = st.radio("Access Level", ["Standard", "Master Alpha 🔓"])

--- ٦. الواجهة الرئيسية ---
st.markdown(f"<h1 style='text-align: center; color: #4facfe;'>{t['title']}</h1>", unsafeallowhtml=True)

أ. التوكن والوكيل
st.markdown(f'<div class="glass-card"><h3>🔒 {t["saden"]}</h3>', unsafeallowhtml=True)
c1, c2 = st.columns([3, 1])
with c1: st.textinput("Token ID", type="password", labelvisibility="collapsed", key="token_main")
with c2: 
    if st.button(t['sync']): 
        st.success("Linked! ✅")
        addtomemory(f"Token Synced: {selected_lang}")
st.markdown('</div>', unsafeallowhtml=True)

ب. الوكيل الذكي (تبويبات)
tab1, tab2, tab3 = st.tabs(["🎙️ Voice", "👋 Sign", "⌨️ Text"])
with tab1: 
    if st.button(f"{t['motto']} (Mic Active)"):
        addtomemory("Voice command engaged")
with tab3: 
    chatval = st.chatinput("Sony-Agent...")
    if chatval: addtomemory(f"Chat: {chatval}")
...
with ca: 
    if st.button(t['car']): 
        ...
        addtomemory("Car Started")
with cb:
    if st.button(t['home']): 
        st.toast("🏠 Welcome Home Mode Active")
        addtomemory("Home Managed")

--- ٧. إبرام الصفقة (الاحتفالية) ---
st.divider()
if st.button(t['buy'], type="primary", usecontainerwidth=True):
    st.balloons(); st.snow()
    addtomemory("Deal Concluded Successfully")
    st.audio("https://www.soundhelix.com/examples/mp3/SoundHelix-Song-8.mp3")
    st.markdown(f"<div class='glass-card' style='text-align: center;'><h2>🏆 {t['success']}</h2><p>Cert: STAR-UNIV-2026-{int(time.time())}</p></div>", unsafeallowhtml=True)
